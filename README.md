...
- **Navigation** links to the `/services`, `/methodology`, `/intellectual-property`, `/education`, `/technology-software` (labeled "Technology"), `/about`, and `/contact` routes, with the header CTA also leading to `/contact`. The desktop link row renders at `xl` and above — measured as the widest breakpoint range where all seven labels fit on one line — and the accessible menu button serves all narrower viewports. No link points to a page that does not exist.
