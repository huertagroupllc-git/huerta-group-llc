...
The header brand mark was formerly listed here as a text-based placeholder. It no longer is: `components/layout/Header.tsx` renders the brand image at `public/brand/hg-mark.png` beside the wordmark, and carries no `TEMPORARY` marker.
...
- **Navigation** — the header carries the prospective-client journey (Services, Methodology, About, Contact) beside the wordmark Home link; the four institutional routes (`/intellectual-property`, `/education`, `/technology-software`, `/managed-services`) remain public and are reached from the mobile menu's secondary group, the About page, and the footer, which links all eight routes. The header CTA also leads to `/contact`. The desktop row renders at `lg` and above; the accessible menu button serves all narrower viewports. This supersedes the `ddr-0011` structure (2027 launch alignment), which had replaced the earlier flat eight-label row that was at measured capacity at `xl` and above (that measurement, and the ceiling it recorded as `gr-0003`, are retained in `docs/launch/launch-readiness.md`). **Any future top-level item remains an owner-authorized structural decision.** No link points to a page that does not exist.
...
1. **[Huerta Group LLC Constitution](institution/governance/constitution/huerta-group-constitution.md)** — the highest internal authority. The ratified text of Articles I–III was supplied through Founder Office authority on 2026-09-10 and is held verbatim beside the canonical record, one file per article (`fd-0040`); constitutional development remains incomplete beyond Article III.
...
form's submission path requires them, and one build-time switch governs
search indexing. Copy `.env.example` to `.env.local` and fill in real
values (never committed):

| Variable | Purpose |
| --- | --- |
| `SITE_INDEXING_ENABLED` | Build-time pre-launch indexing switch, read once in `lib/indexing.ts` (server-only). Only the exact value `true` enables indexing (robots meta `index, follow`, sitemap listed and declared). Absent or any other value — the current pre-launch state — keeps every route `noindex, nofollow`, leaves crawling allowed so the directive is seen, and serves an empty sitemap. Official launch requires setting it to `true` in Vercel (Production) and redeploying; no page edits |
...
- **Pre-launch indexing**: production is deliberately `noindex, nofollow` site-wide until the official launch. The single launch-time switch is `SITE_INDEXING_ENABLED=true` in Vercel (Production) followed by a redeploy — see `lib/indexing.ts` and [`docs/launch/`](docs/launch/official-launch-checklist.md).
