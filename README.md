...
- **Navigation** reaches the `/services`, `/methodology`, `/intellectual-property`, `/education`, `/technology-software`, `/managed-services`, `/about`, and `/contact` routes, with the header CTA also leading to `/contact`. The desktop row renders at `lg` and above as five items — Services, Methodology, the Capabilities disclosure holding the four institutional branches under their full names, About, Contact — plus the CTA; the accessible menu button serves all narrower viewports, and the footer links all eight routes. This is the `ddr-0011` structure, which replaced the earlier flat eight-label row that was at measured capacity at `xl` and above (that measurement, and the ceiling it recorded as `gr-0003`, are retained in `docs/launch/launch-readiness.md`). **Any future top-level item remains an owner-authorized structural decision.** No link points to a page that does not exist.
